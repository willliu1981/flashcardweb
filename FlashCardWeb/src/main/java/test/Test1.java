...
public class Test1 {
	...
	private static void setKeyWithoutAuthority(String userName,
			String... keys) {
	...
	private static void setKeyWithAuthority(String userName,
			String inheritedAuthority, String... keys) {
	...
	private static void setKey(UserType userType, UserBean userBean,
			String... keys) {
		if (userType == UserType.USER) {
			setKeyWithoutAuthority(userBean.getUserName(), keys);
		} else if (userType == UserType.USER_ADMIN) {
			setKeyWithAuthority(userBean.getUserName(),
					userBean.getInheritedAuthority(), keys);
		} else {
