...
public class FlashcardHolderDTOListGroupRenderer
		extends ListGroupRenderere<FlashcardHolderDTO> {
	private ListFacade facade;
	private final String badgeSpanSheet = "text-align: center; padding-left: 20px;"
			+ " padding-right: 20px;";
	private String title;

	public FlashcardHolderDTOListGroupRenderer(ListFacade facade,
			String title) {
		this.facade = facade;
		this.title = title;
	}
	...
	public String getRenderedHeader(Handler handler) {
		...
		StringBuilder sbTitleMsg = Taglibs.getStringBuilder()
				.append("<h3 style='color:cyan;'>").append(title)
				.append("</h3>");
	...
	@Override
	public String getRenderedEachBody(FlashcardHolderDTO model) {
		//delete badge begin
		...
		text.setBody(
				gap + "(" + model.getId() + ")" + gap + gap + "fc-id =" + gap
						+ (model.getFcId() == null ? "N/A"
								: model.getFlashcard().getTerm() + " ("
